...
const WAGE_PRESETS = [15, 18, 22, 30];
...
const Calculator = ({ showcaseMode = false }) => {
  ...
  const [wageMode, setWageMode] = useState(18);
  const [customWage, setCustomWage] = useState("18");

  const hourlyWage = wageMode === "custom" ? (parseFloat(customWage) || 0) : wageMode;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
      ...
      <div className="space-y-6">
        ...
        <div>
          <label className="text-sm font-medium text-white/80 block mb-2">Caregiver Hourly Pay</label>
          <div className="flex flex-wrap gap-2">
            {WAGE_PRESETS.map((w) => (
              <button key={w} onClick={() => setWageMode(w)}
                className={`px-4 py-2 rounded-full text-sm font-medium border transition-all ${wageMode === w ? "bg-white text-black border-white" : "bg-transparent text-white/60 border-white/20 hover:border-white/50 hover:text-white"}`}>
                ${w}/hr
              </button>
            ))}
            <button onClick={() => setWageMode("custom")}
              className={`px-4 py-2 rounded-full text-sm font-medium border transition-all ${wageMode === "custom" ? "bg-white text-black border-white" : "bg-transparent text-white/60 border-white/20 hover:border-white/50 hover:text-white"}`}>
              Custom
            </button>
          </div>
          {wageMode === "custom" && (
            <div className="mt-3 relative">
              ...
              <input type="number" min={1} value={customWage} onChange={(e) => setCustomWage(e.target.value)}
                className="w-full bg-white/10 border border-white/20 rounded-lg pl-7 pr-3 py-2.5 text-white text-sm focus:outline-none focus:border-white/50" />
            </div>
          )}
        </div>
        ...
        <button onClick={() => { setResidents(100); setBathsPerWeek(3); setWageMode(18); setCustomWage("18"); }}
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-xl border border-white/10 bg-white/5 p-4">
          <p className="text-xs text-white/40 font-medium uppercase tracking-wide mb-3">Weekly Cost Comparison</p>
          <div className="flex items-center justify-between gap-4">
            <div className="text-center"><p className="text-xs text-white/40 mb-1">Traditional</p><p className="text-lg font-bold text-white/60 tabular-nums">{fmt(r.tradWeeklyCost)}</p></div>
            <div className="text-white/20 text-xl">→</div>
            <div className="text-center"><p className="text-xs text-white/40 mb-1">Clean Box</p><p className="text-lg font-bold text-white tabular-nums">{fmt(r.cbWeeklyCost)}</p></div>
          </div>
        </div>
        {showcaseMode ? (
